...
#    plt.draw()
    ...
    # Modified Code
    # """
    feedback=["good", "bad", "no feedback", "next state"]
    notRecognise = True
    while notRecognise :
        r = sr.Recognizer() # Obtain audio from the microphone
        with sr.Microphone() as source:
            # Ask the trainer to give a feedback among those proposed
            print("Give your feedback : good, bad, no feedback, next state")
            audio = r.listen(source) # Save the audio feedback  
            
        try :
            # The word Google Speech recognises
            print("Google Speech Recognition thinks you said " + r.recognize_google(audio))
            user_input = feedback.index(r.recognize_google(audio))
            notRecognise = False # We can exit the loop 
        except ValueError:
            # If the word recognised is not in the list of possible feedback,
            # an error message is desplayed
            print(r.recognize_google(audio) + " is not a feedback")
        except sr.UnknownValueError:
            # If the word is not understood by Google Speech, 
